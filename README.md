...
โปรเจกต์ใช้ ADB port เดียวตามค่าที่ตั้งไว้ในช่อง `ADB port` และบันทึก serial ของ MuMu ที่เลือกไว้ใน `config.json` เมื่อเปิดหลายหน้าต่าง โปรแกรมจะไม่สลับไปใช้หน้าต่างอื่นอัตโนมัติ หากต้องการเปลี่ยน instance ให้แก้ port/serial แล้วกดเชื่อมต่อใหม่
...
- **ไม่พบ device:** ตรวจว่า MuMu instance ที่ต้องการใช้ตรงกับ ADB port/serial ที่บันทึกไว้ โปรแกรมตั้งใจไม่สลับไปใช้หน้าต่างอื่น
